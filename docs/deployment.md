...
| Railway (`deploy-railway.yml`)                                | **Supported — current hosted API**       | Auto-deploys the API on push to `main`; smoke-tests the `/api/health` endpoint. This is the API that the hosted frontends point at (`https://stellar-pay-api.up.railway.app`). ⚠️ As of 2026-09 the hosted service returned Railway's 404 for every path — the service must be re-deployed/restored for the live demos to function. Uses testnet by default — **not a mainnet deployment** |
...
| Kubernetes / AKS (`deploy.yml` + `infrastructure/kubernetes`) | **Experimental / production-oriented**   | GHCR images + Kustomize + rollout on `main`. The workflow smoke-tests the deployed API **in-cluster** (kubectl port-forward → `/api/health`) — no external DNS required. Still not the live environment (Vercel/Railway are)                                                                                                                                                               |
...
| Netlify (`netlify.toml`)                                      | **Legacy / not the live path**           | A leftover config for the web app that is not wired into any workflow, not referenced anywhere else in the docs, and **not** the hosted frontend (Vercel is). It now carries the `/api`-prefixed `NEXT_PUBLIC_API_URL` like the Vercel projects; treat it as unmaintained until it has a CI job of its own                                                                                 |
...
The alerting pipeline was **verified with an intentional-failure drill**
(2026-09-09): with the API up and Prometheus scraping, killing the API process
flipped the target to `down` and the `ApiDown` rule to **firing** within the
`for` window, and restarting the API cleared the alert and restored the target
to `up` — detection → firing → recovery all confirmed. See
`infrastructure/monitoring/README.md` → Failure drill for the exact steps.
