...
| Soroban contracts (6)                                     | **DEV (contract-level)**      | Rust unit tests; all 6 deployed and re-verified live on testnet 2026-09-11 (`getLedgerEntries`).The `payment` route is **live-verified end-to-end**, and the `escrow`/`treasury`/`subscriptions`/`invoices`/`merchant` routes (via `ContractIntegrationService` + indexer reconciliation) are **live-verified** by the 28/28 `tests/e2e/contracts-flow.mjs` run on 2026-09-11. Remaining gap: `treasury`/`subscriptions` have no JS/TS unit tests        |
